...
import { ensureTrialSubscription, latestSubscription, billingAccess, STATUS_LABELS, PLAN_LABELS, planPriceCopy, customerPortalUrl, billingMode, manualInvoiceEmail } from '@/lib/billing';
...
export default async function BillingPage() {
  ...
  const invoiceEmail = manualInvoiceEmail();
  return <Shell>
    <div className="mb-6">
    ...
    {mode === 'MANUAL_INVOICE' ? <section className="mt-6 card p-5" data-testid="manual-invoice-policy">
      <h2 className="text-xl font-black">Founding-customer billing: manual invoice</h2>
      <p className="mt-2 text-sm text-slate-600">Billing is handled by invoice during the pilot and founding-customer phase. This is an explicit operating policy, not an unfinished checkout promise.</p>
      <p className="mt-3 text-sm font-bold">Billing contact: {invoiceEmail}</p>
    </section> : <>
      ...
      <section className="mt-6 card p-5">
        ...
        <p className="mt-2 text-sm text-slate-600">Use Stripe customer portal for card updates, cancellation, and invoices.</p>
