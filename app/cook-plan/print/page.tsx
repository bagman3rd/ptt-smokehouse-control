...
export default async function CookPlanPrintPage({ searchParams }: { searchParams?: { planId?: string } }) {
  ...
  return (
    <main className="mx-auto max-w-5xl bg-white p-8 text-slate-950 print:p-4">
      <style>{`@media print { .no-print { display: none; } body { background: white; } table { page-break-inside: avoid; } }`}</style>
      ...
      <header className="border-b-4 border-slate-900 pb-4">
        <h1 className="text-3xl font-black">Smokehouse Daily Load Plan</h1>
      ...
      <table className="mt-6 w-full border-collapse text-sm">
        ...
        <tbody>{plan.items.map((item) => <tr key={item.id} className="border-b border-slate-300 align-top"><td className="p-2 font-black">{item.protein.name}</td><td className="p-2 text-xl font-black">{item.approvedCookUnits ?? item.recommendedCookUnits} {displayUnit(item.protein.name, item.protein.inputUnit)}</td><td className="p-2">{item.usableLeftoverUnits} {displayUnit(item.protein.name, item.protein.inputUnit)} / {item.usableLeftoverLb} lb</td><td className="p-2">{timing(item.protein.name)}</td><td className="p-2">{item.overrideReason || item.notes || '—'}</td></tr>)}</tbody>
      </table>

      <section className="mt-8 rounded-xl border-2 border-slate-900 p-4">
        <div className="font-black">Manager Signoff</div>
        <div className="mt-6 grid gap-8 md:grid-cols-2"><div>KM: __________________________</div><div>Time: __________________________</div></div>
      </section>
    </main>
  );
}
