            ...
            <div>
              ...
              <div className="mt-3 grid gap-2 text-sm text-slate-600 md:grid-cols-3">
                ...
                <div className={timing.usesCredit ? "rounded-xl bg-amber-50 p-3" : "rounded-xl bg-slate-50 p-3"}>
                  ...
                  {noPriorEodData ? <strong className="text-base text-amber-900">no data, check hot box</strong> : <><strong data-testid={`prior-eod-credit-value-${item.protein.code || item.protein.id}`} className={timing.usesCredit ? "text-xl text-amber-900" : "text-xl text-slate-500"}>{item.usableLeftoverUnits}</strong> {displayUnit(item.protein.name, item.protein.inputUnit)} <span className="text-xs">/ {item.usableLeftoverLb} lb</span></>}
                </div>
