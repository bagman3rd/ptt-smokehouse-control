...
export default async function SystemPage() {
  ...
  return <Shell>
    ...
    <div className="rounded-2xl border border-amber-300 bg-amber-50 p-5 text-amber-900">
      ...
      <p className="mt-1 text-sm">This deployment is configured for <strong>{migrationMode}</strong>. Deploy Build 4.3.1 to production only after the failed migration state is repaired/baselined on staging and the staging checks pass.</p>
    </div>
    ...
    <section className="mt-6 grid gap-4 lg:grid-cols-3">
      <div className="rounded-2xl border border-red-200 bg-red-50 p-5 text-red-900">
        ...
        <p className="mt-2 text-sm">The previous live database had a failed Prisma migration record. Build 4.3.1 makes migrate deploy the active path; use the runbook on staging before production deployment.</p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <p className="mt-2 text-sm text-slate-600">Build 4.3.1 includes a weekly backup endpoint plus a cron runner script. Configure CRON_SECRET and BACKUP_APP_URL for scheduled backups.</p>
    ...
    <section className="card mt-6 p-5">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        {[
          ...
          ['MIGRATION_REPAIR_RUNBOOK', 'Migration repair runbook rehearsal', 'Run Build 4.3.1 migration repair runbook against staging']
        ].map(([type, label, hint]) => (
    ...
    <section className="card mt-6 p-5">
      ...
      <p className="mt-3 text-sm font-bold text-red-700">Do not deploy Build 4.3.1 to production until the staging rehearsal passes and is recorded above.</p>
    </section>
