# Migration Repair Runbook — Build 4.1.0

Build 4.1.0 switches normal deploys back to `prisma migrate deploy`. Do not run it against production until this runbook has passed on a staging copy.

## Why this matters

The live Render database previously had a failed Prisma migration record. Running `prisma migrate deploy` against a database with a failed migration record returns Prisma P3009 and stops deployment. Build 4.1.0 removes the db-push recovery path, so the failed migration must be resolved or baselined first.
