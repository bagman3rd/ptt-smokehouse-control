...
// Build 5.8.0 catalog policy:
// 1) No estimated capacity numbers.
// 2) Count fields are only used when the manufacturer publishes the same operational unit.
// 3) Published pound, range, whole-chicken, or half-chicken capacities are preserved in officialCapacityText but are not auto-loaded as brisket/pork/rib/chicken-breast planning counts.
export const smokerCatalogSeeds: SmokerCatalogSeed[] = [
  // Ole Hickory Pits. Brisket, butt, and rib counts are official count capacities. Whole-chicken capacity is official text only, not chicken-breast capacity.
