// Build 11.0.1 — daily data-retention cron runner.
// Invokes GET /api/cron/retention with the shared CRON_SECRET.
...
const endpoint = `${appUrl}/api/cron/retention`;
console.log(`Running retention purge against ${endpoint}`);
const res = await fetch(endpoint, { method: 'GET', headers: { authorization: `Bearer ${secret}` } });
const text = await res.text();
if (!res.ok) {
  console.error(`Retention job failed with HTTP ${res.status}`);
  console.error(text);
  process.exit(1);
}
console.log(text);
