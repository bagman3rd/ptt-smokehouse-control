...
function updatePackage() {
  ...
  data.scripts["master-data:db-readiness"] = "node scripts/database-readiness-11.2.0.mjs";
  data.scripts["build:11.2.0"] = "node scripts/apply-build-11.2.0.mjs";
  fs.writeFileSync(packagePath, `${JSON.stringify(data, null, 2)}\n`, "utf8");
  console.log("UPDATED — package.json version and Build 11.2.0 scripts.");
}
...
function verifyRender() {
  ...
  if (!/key:\s*APP_BUILD_VERSION\s*\n\s*value:\s*"11\.2\.0"/m.test(render)) {
    throw new Error("render.yaml does not declare APP_BUILD_VERSION 11.2.0.");
  }
  console.log("PASS — render.yaml declares Build 11.2.0.");
}
...
run("scripts/database-readiness-11.2.0.mjs");

console.log("\nBuild 11.2.0 overlay applied, Setup Center generated, and master-data evidence produced.");
