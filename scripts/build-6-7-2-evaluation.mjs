...
assert.equal(pkg.version, '6.7.2');
...
assert.ok(quick.includes('step="1"'));
assert.ok(quick.includes('Number.isInteger(row.sealedUnopenedUnits)'));
assert.ok(read('lib/validators.ts').includes('sealedUnopenedUnits: z.coerce.number().int().min(0).optional()'));
assert.ok(eodRoute.includes('!Number.isInteger(sealedUnopenedUnits)'));
assert.ok(read('prisma/schema.prisma').includes('sealedUnopenedUnits Int @default(0)'));
assert.ok(existsSync('prisma/migrations/20260712003000_build_671_integer_sealed_counts/migration.sql'));
assert.ok(read('prisma/schema.prisma').includes('openedMeatLb'));

console.log('Build 6.7.2 evaluation passed.');
