...
const expectedBuild = process.env.EXPECTED_BUILD || '7.7.1';
...
for (const endpoint of ['/api/health', '/api/health/db']) {
  await retry(endpoint, async () => {
    const response = await fetch(`${baseUrl}${endpoint}`, { headers: { 'user-agent': 'PTT-Build-7.7.1-Monitor' }, signal: AbortSignal.timeout(20000) });
...
await retry('authenticated smoke transaction', async () => {
  ...
  const login = await fetch(`${baseUrl}/api/login`, {
    ...
    headers: { 'content-type': 'application/x-www-form-urlencoded', 'user-agent': 'PTT-Build-7.7.1-Monitor' },
  ...
  const dashboard = await fetch(`${baseUrl}/today`, { headers: { cookie, 'user-agent': 'PTT-Build-7.7.1-Monitor' }, redirect: 'manual', signal: AbortSignal.timeout(30000) });
  ...
  const canary = await fetch(`${baseUrl}/api/cook-plan`, { method: 'POST', headers: { cookie, 'content-type': 'application/json', 'user-agent': 'PTT-Build-7.7.1-Monitor' }, body: JSON.stringify({ serviceDate: 'invalid-canary-date', eventMultiplier: 1 }), signal: AbortSignal.timeout(30000) });
