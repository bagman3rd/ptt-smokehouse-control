...
const expectedBuild = process.env.EXPECTED_BUILD || '11.0.1';
...
for (const endpoint of ['/api/health', '/api/health/db']) {
  await retry(endpoint, async () => {
    const response = await fetch(`${baseUrl}${endpoint}`, { headers: { 'user-agent': 'PTT-Build-11.0.1-Monitor' }, signal: AbortSignal.timeout(20000) });
...
await retry('authenticated smoke transaction', async () => {
  ...
  const login = await fetch(`${baseUrl}/api/login`, {
    ...
    headers: { 'content-type': 'application/x-www-form-urlencoded', 'user-agent': 'PTT-Build-11.0.1-Monitor' },
    signal: AbortSignal.timeout(30000)
  });
  const cookie = login.headers.get('set-cookie')?.split(';')[0] || '';
  const location = login.headers.get('location') || '';
  if (![302, 303, 307, 308].includes(login.status) || !cookie || !location.includes('/today')) {
    throw new Error(`Login smoke failed with status ${login.status}`);
  }
  const dashboard = await fetch(`${baseUrl}/today`, { headers: { cookie, 'user-agent': 'PTT-Build-11.0.1-Monitor' }, redirect: 'manual', signal: AbortSignal.timeout(30000) });
  const html = await dashboard.text();
  if (dashboard.status !== 200 || !html.includes('Smokehouse Control')) throw new Error(`Authenticated Today smoke failed with status ${dashboard.status}`);
  console.log('PASS authenticated login and Today transaction');
  const canary = await fetch(`${baseUrl}/api/cook-plan`, { method: 'POST', headers: { cookie, 'content-type': 'application/json', 'user-agent': 'PTT-Build-11.0.1-Monitor' }, body: JSON.stringify({ serviceDate: 'invalid-canary-date', eventMultiplier: 1 }), signal: AbortSignal.timeout(30000) });
  if (canary.status !== 400) throw new Error(`Cook-plan validation canary expected 400, received ${canary.status}`);
  console.log('PASS authenticated non-mutating cook-plan validation canary');
});
