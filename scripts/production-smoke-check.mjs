...
const expectedBuild = process.env.EXPECTED_BUILD || '7.4.0';
...
for (const endpoint of ['/api/health', '/api/health/db']) {
  await retry(endpoint, async () => {
    const response = await fetch(`${baseUrl}${endpoint}`, { headers: { 'user-agent': 'PTT-Build-7.4.0-Monitor' }, signal: AbortSignal.timeout(20000) });
...
await retry('authenticated smoke transaction', async () => {
  ...
  const login = await fetch(`${baseUrl}/api/login`, {
    ...
    headers: { 'content-type': 'application/x-www-form-urlencoded', 'user-agent': 'PTT-Build-7.4.0-Monitor' },
  ...
  const dashboard = await fetch(`${baseUrl}/dashboard`, { headers: { cookie, 'user-agent': 'PTT-Build-7.4.0-Monitor' }, redirect: 'manual', signal: AbortSignal.timeout(30000) });
