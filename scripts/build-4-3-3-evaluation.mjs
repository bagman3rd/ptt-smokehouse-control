...
assert(pkg.version === '4.3.3', 'package.json version should be 4.3.3');
...
assert(nav.includes('Build 4.3.3'), 'Nav badge should show Build 4.3.3');
...
assert(existsSync('prisma/migrations/20260712000200_build_430_security_tenant_guard/migration.sql'), 'Build 4.3.3 migration should exist');
...
assert(backup.includes("build: '4.3.3'"), 'manual backup export should identify Build 4.3.3');

assert(existsSync('TEST_REPORT_BUILD_4_3_3.md'), 'Build 4.3.3 test report should exist');
assert(existsSync('TENANT_SECURITY_BUILD_4_3_3.md'), 'Build 4.3.3 tenant/security notes should exist');

console.log('Build 4.3.3 evaluation checks completed.');
