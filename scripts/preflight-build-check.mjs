...
assert(version === '4.5.0', 'package.json version must be 4.5.0 for this build');
...
const noLongerAllowed = [
  'Build 4.4.0',
  'build-4-4-0-evaluation',
  'prisma db push &&',
  'db push --accept-data-loss',
  'tenantOrLegacyWhere',
  'tenantWhere('
];
const projectText = ['README.md', 'package.json', 'components/Nav.tsx', '.github/workflows/ci.yml']
  .map((file) => read(file))
  .join('\n');
for (const token of noLongerAllowed.slice(0, 4)) {
  assert(!projectText.includes(token), `obsolete token still present: ${token}`);
}

console.log('Build 4.5.0 preflight checks completed.');
