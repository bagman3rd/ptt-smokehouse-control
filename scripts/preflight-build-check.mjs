...
assert(version === '11.0.2', 'package.json version must be 11.0.2 for this build');
...
assert(existsSync('pnpm-lock.yaml'), 'committed pnpm-lock.yaml must exist');
assert(!existsSync('.github/workflows/lockfile-bootstrap.yml'), 'temporary lockfile bootstrap workflow must be removed');
...
assert(!existsSync('docs/archive'), 'historical release archive must not be included in deployable artifact');
assert(!existsSync('scripts/archive'), 'historical evaluation archive must not be included in deployable artifact');
...
console.log('Build 11.0.2 preflight checks completed.');
