...
function updatePackage() {
  ...
  data.scripts["inventory-control:workbench"] =
    "node scripts/generate-inventory-workbench-11.6.0.mjs";
  ...
  data.scripts["build:11.6.0"] =
    "node scripts/apply-build-11.6.0.mjs";
  fs.writeFileSync(
    packagePath,
  ...
  console.log(
    "UPDATED — package.json version and Build 11.6.0 scripts.",
  );
}
...
function verifyRender() {
  ...
  if (
    !/key:\s*APP_BUILD_VERSION\s*\n\s*value:\s*"11\.6\.0"/m.test(
      render,
    )
  ) {
    throw new Error(
      "render.yaml does not declare APP_BUILD_VERSION 11.6.0.",
    );
  }
  console.log("PASS — render.yaml declares Build 11.6.0.");
}
...
console.log(
  "\nBuild 11.6.0 overlay applied, Inventory Control Lab generated, and reconciliation evidence produced.",
);
