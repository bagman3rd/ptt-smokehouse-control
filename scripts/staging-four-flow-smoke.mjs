...
const health=await fetch(base+'/api/health'); const hj=await health.json(); if(!health.ok||hj.build!=='10.0.0') throw new Error(`staging build mismatch: ${JSON.stringify(hj)}`);
let cookie='';
