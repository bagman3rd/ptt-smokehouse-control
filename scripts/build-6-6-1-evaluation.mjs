...
assert.ok(quick.includes("name.includes('brisket')"), 'legacy brisket name fallback required');
assert.ok(quick.includes("name.includes('pork') || name.includes('butt')"), 'legacy pork name fallback required');
assert.ok(quick.includes("name.includes('chicken') || name.includes('breast')"), 'legacy chicken name fallback required');
assert.ok(quick.includes("name.includes('rib')"), 'legacy rib name fallback required');
assert.ok(!quick.includes('disabled={isLocked || !protein}'), 'quick EOD inputs must not be disabled by missing protein codes');

console.log('Build 6.6.1 evaluation passed.');
