...
function updatePackage() {
  ...
  Object.assign(data.scripts, {
    ...
    "hardening:workbench":
      "node scripts/generate-hardening-workbench-11.9.0.mjs",
    "hardening:evidence":
    ...
    "build:11.9.0":
      "node scripts/apply-build-11.9.0.mjs",
  });
  fs.writeFileSync(
    packagePath,
  ...
  console.log(
    "UPDATED — package.json version and Build 11.9.0 scripts.",
  );
}
...
function verifyRender() {
  ...
  if (
    !/key:\s*APP_BUILD_VERSION\s*\n\s*value:\s*"11\.9\.0"/m.test(
      render,
    )
  ) {
    throw new Error(
      "render.yaml does not declare APP_BUILD_VERSION 11.9.0.",
    );
  }
  const webCount = (
  ...
  console.log(
    "PASS — render.yaml declares Build 11.9.0 with one web service and zero cron services.",
  );
}
...
console.log(
  "\nBuild 11.9.0 overlay applied, hardening lab generated, and release-gate evidence produced.",
);
