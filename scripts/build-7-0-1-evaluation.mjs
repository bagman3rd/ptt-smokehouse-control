...
assert.match(pkg.scripts.build, /prisma:generate:fresh/);
assert.match(pkg.scripts.build, /verify:prisma-client/);
assert.match(pkg.scripts['render-build'], /prisma:generate:fresh/);
...
assert.match(read('app/api/health/route.ts'), /7\.0\.1/);
console.log('Build 7.0.1 evaluation passed.');
