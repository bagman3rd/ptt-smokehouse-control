...
function updatePackage() {
  ...
  Object.assign(data.scripts, {
    ...
    "build:12.1.0":
      "node scripts/apply-build-12.1.0.mjs",
  });
  fs.writeFileSync(
    packagePath,
  ...
  console.log(
    "UPDATED — package.json version and Build 12.1.0 scripts.",
  );
}
...
function verifyRender() {
  ...
  if (
    ...
    throw new Error(
      "render.yaml does not declare APP_BUILD_VERSION 12.1.0.",
    );
  }
  const webCount = (
  ...
  console.log(
    "PASS — render.yaml declares Build 12.1.0 with one web service and zero cron services.",
  );
}
...
console.log(
  "\nBuild 12.1.0 overlay applied. Multi-location deterministic evidence is complete; durable schema migration and deployed UAT remain pending.",
);
