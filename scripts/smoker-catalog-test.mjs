...
const banned = ['sourceConfidence:\'ESTIMATED\'','capacities estimated','estimated from','wholeChickenToBreasts','Stored chicken capacity is','breast-equivalent'];
...
for (const token of ['rackCount:12','brisketCapacity:8','porkCapacity:24','ribCapacity:24','chickenCapacity:null',"officialCapacityText:'12 racks; 24 pork butts; 24 St. Louis ribs; 32 whole chickens; 8 beef briskets'"]) {
  if (!mlr.includes(token)) throw new Error(`MLR-150 fixture failed: ${token}`);
}
const eledx = rowFor('EL-ED/X');
for (const token of ['rackCount:12','rackWidthIn:18','rackDepthIn:48','cookingAreaSqIn:10368','brisketCapacity:40','porkCapacity:80','ribCapacity:105','chickenCapacity:null']) {
  if (!eledx.includes(token)) throw new Error(`EL-ED/X fixture failed: ${token}`);
}
const sm160 = rowFor('SM160');
for (const token of ['brisketCapacity:null',"brisketCapacityUnit:'LB'",'porkCapacity:null',"porkCapacityUnit:'LB'",'ribCapacity:null',"ribCapacityUnit:'LB'",'chickenCapacity:null',"officialCapacityText:'120 lb pork butts; 100 lb brisket; 50 lb ribs; 20 whole chickens'"]) {
...
console.log(`Build 5.8.0 smoker catalog regression checks passed with ${modelCount} manufacturer rows.`);
