...
assert(pkg.version === '4.3.0', 'package.json version should be 4.3.0');
...
assert(nav.includes('Build 4.3.0'), 'Nav badge should show Build 4.3.0');
...
assert(existsSync('prisma/migrations/20260712000200_build_430_security_tenant_guard/migration.sql'), 'Build 4.3.0 migration should exist');
...
assert(backup.includes("build: '4.3.0'"), 'manual backup export should identify Build 4.3.0');

assert(existsSync('TEST_REPORT_BUILD_4_3_0.md'), 'Build 4.3.0 test report should exist');
assert(existsSync('TENANT_SECURITY_BUILD_4_3_0.md'), 'Build 4.3.0 tenant/security notes should exist');

console.log('Build 4.3.0 evaluation checks completed.');
