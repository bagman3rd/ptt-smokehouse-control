# Build 11.0.2 Release Gate

> **11.0.2 is a hotfix over 11.0.1.** It fixes a TypeScript type error that
> failed `next build` on Render: `lib/notifications/dispatch.ts` applied `as
> const` to a ternary expression (TS1355 — `as const` may only be applied to
> literals, arrays, or objects). Fixed by giving `mapCategory` an explicit
> return type. A new `scripts/typescript-antipattern-check.mjs` guard (wired
> into `ci:test` and `test:compliance`) now catches this class of build-breaking
> TS syntax error offline, before deploy.
>
> **11.0.1** fixed a broken relative import in the confirm-delete page and added
> `scripts/import-resolution-check.mjs`.
