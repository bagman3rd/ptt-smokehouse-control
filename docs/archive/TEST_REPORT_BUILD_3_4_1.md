...
- Preserves Build 3.5.0 UI cleanup and liquor/food/smoked-meat visibility.
...
`prisma migrate deploy` refuses to apply new migrations when the target database contains a failed migration record. This is correct Prisma behavior. Since this database was evolved through previous MVP `db push` builds, Build 3.5.0 uses the safer recovery path for this existing Render database.
