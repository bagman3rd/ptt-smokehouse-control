# PTT Smokehouse Control — Build 9.7.0

## Build 9.7.0 release safety update

Build 9.7.0 hardens deployment and demo behavior. The POS foundation migration is required to keep every additive column repair idempotent, including `PosConnection.lastError`. CI now performs both a fresh migration replay and a prior-schema upgrade replay against a database containing a legacy partial `PosConnection` table. Archer retains the approved uncensored identity response by default, while `ARCHER_DEMO_MODE=true` replaces the final profanity for customer demonstrations.
