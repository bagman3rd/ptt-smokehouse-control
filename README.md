# PTT Smokehouse Control — Build 7.5.2

Build 7.5.2 is a full recovery release rebuilt from the last clean application baseline, Build 7.0.1.

## Build 7.5.2 navigation reliability fix

The top navigation uses the compact dropdown layout from the earlier stable releases. Operations, Insights, Admin, and Help open one menu at a time, close on outside click or Escape, and preserve role-based access. Today remains the default landing page, while Logout and restaurant switching remain ordinary forms.
...
Replace the repository contents with the complete extracted Build 7.5.2 tree. Do not merge only selected files into an older working directory. Commit deletions as well as additions so stale files cannot remain in GitHub.
